							...
							<Flex gap={4} key={i} as={Link} to={"/stocks/" + stock.symbol}>
								...
								<Stack flex="0.5" alignItems={"end"}>
									...
									<Tag
										...
										w={"fit-content"}
										colorScheme={
											(((stock.regularMarketPrice / stock.regularMarketPreviousClose)*100)-100) >= 0 ? "green" : "red"
										}
									>
										{(((stock.regularMarketPrice / stock.regularMarketPreviousClose)*100)-100) > 0 ? "+" : ""}
										{(((stock.regularMarketPrice / stock.regularMarketPreviousClose)*100)-100).toFixed(2)}%
									</Tag>
								</Stack>
							</Flex>
						))}
